...
首先是在对象实例化之前调用``InstantiationAwareBeanPostProcessor``接口的postProcessBeforeInstantiation方法，如果在这个方法里返回不为空的话能够直接return，截断接下来的Bean实例的默认创建。也就是在这里直接返回一个代理对象。AOP就是基于这实现的。
